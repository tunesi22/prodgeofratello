...
export function AnimatedFooter() {
  ...
  useEffect(() => {
    ...
    function revealFooter() {
      if (!footerTimeline.current) {
        return;
      }

      if (hideFooterTimer.current) {
        window.clearTimeout(hideFooterTimer.current);
      }
    ...
    function handleWheelGesture(event: WheelEvent) {
      if (event.deltaY > 2) {
        revealFooter();
    ...
    function handleKeyDown(event: KeyboardEvent) {
      if (["ArrowDown", "PageDown", "Space"].includes(event.code)) {
        revealFooter();
      } else if (["ArrowUp", "PageUp", "Home"].includes(event.code)) {
        hideFooter(0);
      }
    }
