...
/** Stat tile used inside the preview. */
function MetricTile({
  label,
  value,
  delta,
  ...
  return (
    <div className="flex flex-col gap-1 rounded-2xl border border-neutral-primary bg-card p-4">
      <span className="text-[13px] font-medium text-neutral-500">{label}</span>
      <div className="flex items-end gap-2">
        <span className="text-[32px] font-semibold leading-none tracking-tight text-primary">{value}</span>
        {delta && (
          <span
            className={cn(
              'mb-0.5 inline-flex items-center gap-0.5 rounded-full px-1.5 py-0.5 text-[12px] font-semibold',
              delta.dir === 'up' ? 'bg-display-brand text-brand-token' : 'bg-display-error text-error-token',
            )}
          >
            {delta.dir === 'up' ? <TrendUp className="size-3" weight="bold" /> : <TrendDown className="size-3" weight="bold" />}
            {delta.value}
          </span>
        )}
      </div>
      {caption && <span className="text-[12px] text-neutral-500">{caption}</span>}
      {children}
    </div>
  )
}

/** Hero-overlapping product preview: the page's flagship "screenshot". */
export function DashboardPreview({ className }: { className?: string }): ReactElement {
  const c = useMockup()
  return (
    <BrowserChrome label={c.dashboard} status={<MockChip dot>{c.live}</MockChip>} className={className}>
      <div className="grid gap-4 lg:grid-cols-12">
        {/* Left: headline metrics */}
        <div className="flex flex-col gap-4 lg:col-span-7">
          <div className="grid grid-cols-2 gap-4">
            <MetricTile label={c.mentionRate} value="62%" delta={{ dir: 'up', value: '+8' }} caption={c.vsLastWeek}>
              <TrendSparkline className="mt-2" />
            </MetricTile>
            <MetricTile label={c.totalQueries} value="1.000" caption={c.tested5x}>
              <div className="mt-2 flex items-center gap-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <span key={i} className="h-1.5 flex-1 rounded-full bg-brand-300" />
                ))}
              </div>
            </MetricTile>
          </div>
          ...
          <div className="rounded-2xl border border-neutral-primary bg-card p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <MockChip tone="neutral">{c.competitors}</MockChip>
            </div>
            <div className="flex h-3 w-full overflow-hidden rounded-full">
              <span className="bg-brand-600" style={{ width: '41%' }} />
              <span className="bg-brand-300" style={{ width: '33%' }} />
              <span className="bg-neutral-200" style={{ width: '26%' }} />
            </div>
            <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-[12px]">
              <Legend color="bg-brand-600" label="Solea" value="41%" />
              <Legend color="bg-brand-300" label={c.competitorA} value="33%" />
              <Legend color="bg-neutral-200" label={c.others} value="26%" />
            </div>
          </div>
        </div>

        {/* Right: per-model breakdown */}
        <div className="flex flex-col gap-3 rounded-2xl border border-neutral-primary bg-card p-4 lg:col-span-5">
          <div className="flex items-center justify-between">
            <span className="text-[13px] font-medium text-neutral-500">{c.perModel}</span>
            <MockChip dot>{c.auto}</MockChip>
          </div>
          <ModelRow />
          <div className="mt-1 flex items-center gap-2 rounded-xl bg-secondary px-3 py-2 text-[12px] text-secondary">
            <CheckCircle className="size-4 text-icon-brand" weight="fill" />
            {c.scannedDone}
          </div>
        </div>
      </div>
...
function Legend({ color, label, value }: { color: string; label: string; value: string }): ReactElement {
  return (
