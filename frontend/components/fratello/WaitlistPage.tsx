...
export function WaitlistPage() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.from("[data-gsap='waitlist-form']", {
        y: 22,
        scale: 0.98,
        duration: 0.75,
        delay: 0.5,
  ...
  useEffect(() => {
    return () => {
      if (restoreFormTimer.current) {
        window.clearTimeout(restoreFormTimer.current);
      }
    };
  }, []);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!normalizedEmail) {
      setValidationMessage("Masukkan email dulu.");
      animateValidationError();
      return;
    }

    if (normalizedEmail.length > 254) {
      setValidationMessage("Email terlalu panjang.");
      animateValidationError();
      return;
    }

    if (/\s/.test(normalizedEmail)) {
      setValidationMessage("Email tidak boleh memakai spasi.");
      animateValidationError();
      return;
    }

    if (!emailPattern.test(normalizedEmail)) {
      setValidationMessage("Masukkan format email yang valid.");
      animateValidationError();
      return;
    }
